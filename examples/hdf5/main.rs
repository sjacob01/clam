    ...
    T: Number,              // For converting Tr and Te away from bool for kosarak data.
{
    ...
    // Adding a 2% buffer to search radius to test change in recall
    let search_radii: Vec<_> = distances
        .into_iter()
        .map(|row| clam::utils::helpers::arg_max(&row).1)
        .map(|v| D::from(v.as_f64() * 1.02).unwrap())
        .collect();
    ...
    let partition_criteria = clam::PartitionCriteria::new(true)
        ...
        .with_custom(Box::new(MinRadius {
            threshold: D::from(min_radius.as_f64() / 10.).unwrap(),
        }));
...
fn main() -> Result<(), String> {
    ...
    let results = [
        ...
        search::<f32, f32, i32, f32, f32>("glove-50", "cosine", 100),
        search::<f32, f32, i32, f32, f32>("glove-100", "cosine", 100),
        search::<f32, f32, i32, f32, f32>("glove-200", "cosine", 100),
        // search::<bool, bool, i32, f32, u8>("kosarak", "jaccard", 100),
        search::<f32, f32, i32, f32, f32>("mnist", "euclidean", 100),
        search::<f32, f32, i32, f32, f32>("nytimes", "cosine", 100),
        search::<f32, f32, i32, f32, f32>("sift", "euclidean", 100),
        search::<f32, f64, i32, f32, f32>("lastfm", "cosine", 100),
    ];
    println!(
